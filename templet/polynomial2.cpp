...
namespace polynomial  {
    ...
    poly mod(const poly &a,const poly &b) {
        return (a-mul(div(a,b),b)).normalize();
    }

    LL sgn(LL a)    {   return (a&1)?modu-1:1;  }
    LL res(const poly &a,const poly &b) {
        if (!a.size()||!b.size()) return 0;
        if (a.size()==1||b.size()==1)   
            return pw(a.back(),b.deg())*pw(b.back(),a.deg())%modu;
        poly t=b/b.back();
        return sgn(a.deg()*b.deg())*res(t,a%t)%modu*pw(b.back(),a.deg())%modu;
        // return sgn(a.deg()*b.deg())*res(t,mod(a,t))%modu*pw(b.back(),a.deg())%modu;
    }
}
